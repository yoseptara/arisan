...
export default function CoordinatorProposalPage({
  params: { address }
}: {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      const tx = await connectedGroupContract.proposeNewCoordinator(
        coordinatorAddress
      );
      ...
      >
        <div className="mb-6">
          <label
            ...
            htmlFor="coordinatorAddress"
          >
            Alamat (Wallet Address) Koordinator
          </label>
          <input
            className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline"
            id="coordinatorAddress"
            type="text"
            placeholder="Masukkan alamat / wallet address koordinator baru"
            minLength={3}
            required={true}
