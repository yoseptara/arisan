...
//     function getGroups() external view returns (GroupData[] memory) {
//         GroupData[] memory groups = new GroupData[](groupAddreseses.length);
...
//         // replace the member to delete with the last member in the list
//         uint256 index = memberIndices[msg.sender] - 1; // subtract 1 because we added 1 when storing
//         address lastAddress = memberAddresses[memberAddresses.length - 1];
//         memberAddresses[index] = lastAddress;
//         memberIndices[lastAddress] = index + 1; // add 1 because we subtracted 1 for the lookup
//         // remove the last member in the list
//         memberAddresses.pop();
...
//         // if (sender.hasJoinedPeriod == false) {
//         //     sender.hasJoinedPeriod = true;
//         // }
...
//         // Push an empty Period struct to the storage array
//         periods.push();

//         // Get a reference to the newly added Period struct in storage
//         Period storage newPeriod = _getLastPeriod();

//         // Update the properties of the newPeriod
//         newPeriod.startedAt = block.timestamp;
//         newPeriod.contributionAmountInWei = contributionAmountInWei;
//         newPeriod.rounds.push();
//         // newPeriod.dueWinners = new address[](0);
...
//     // return "true" if all voters have approved the proposal
...
//     // function proposeNewUint(
//     //     uint newValue,
//     //     ProposalCategory category
//     // ) external memberOnly {
//     //     _propose(category);

//     //     uintProposalValues[proposals.length - 1] = newValue;
//     // }
...
//     // CoordinatorRewardPercentageProposal
